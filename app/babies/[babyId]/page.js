...
async function getReadings(babyId) {
  try {
    ...
    const response = await fetch(`${API_URL}/babies/${babyId}/readings/`, {
      cache: "no-store",
    });
...
export async function generateMetadata({ params }) {
  ...
  if (babyResult.error || !babyResult.data) {
    return {
      ...
      description: "Baby health records could not be loaded.",
    };
  }
  ...
  return {
    ...
    openGraph: {
      ...
      description: `View glucose readings and medicine records for ${baby.name}.`,
    },
  };
}
      ...
      <main className="flex min-h-screen items-center justify-center bg-gradient-to-br from-sky-100 via-pink-50 to-purple-100 px-4">
        <div className="w-full max-w-md rounded-3xl bg-white p-8 text-center shadow-xl">
          ...
          <div className="mt-6 rounded-2xl bg-sky-50 p-4">
            <p className="text-sm text-slate-500">Baby ID</p>
      ...
      <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Baby ID:{" "}
            <span className="font-semibold text-slate-700">{baby.baby_id}</span>
          </p>
        </div>
